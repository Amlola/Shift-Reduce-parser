...
class LR_Parser final {
    ...
    private:
        ...
        const std::unordered_map<Condition, Action, ConditionHash> action_table = {
            ...
            {{.cur_cond = 14, .token = LexicalAnalyzer::Lexer::Tokens::MUL},      {.next_cond = -1, .action = ActionType::REDUCE_E_TO_E_SUB_T}},
            {{.cur_cond = 14, .token = LexicalAnalyzer::Lexer::Tokens::DIV},      {.next_cond = -1, .action = ActionType::REDUCE_E_TO_E_SUB_T}},
